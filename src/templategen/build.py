...
def build_project(request: BuildRequest) -> BuildResult:
    ...
    _load_file_contents(document_tree, request.repo_root)
...
def _load_file_contents(directory: DirectoryNode, repo_root: Path) -> None:
    for file_node in directory.files:
        source_path = (repo_root / file_node.relative_path).resolve()
        file_node.content = source_path.read_text(encoding="utf-8", errors="replace")

    for child in directory.directories:
        _load_file_contents(child, repo_root)
