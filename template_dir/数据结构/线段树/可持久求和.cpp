...
void modify(int &now, int pre, int l, int r, int ql, int qr, int val) {
    ...
    int mid = (l + r) >> 1;
    if (qr <= mid) modify(lch[now], lch[now], l, mid, ql, qr, val);
    else if (ql > mid) modify(rch[now], rch[now], mid + 1, r, ql, qr, val);
    else {
...
ll query(int p, int l, int r, int ql, int qr) {
    ...
    int mid = (l + r) >> 1;
    ll tagVal = tag[p];
    if (qr <= mid) return tagVal + query(lch[p], l, mid, ql, qr);
    else if (ql > mid) return tagVal + query(rch[p], mid + 1, r, ql, qr);
    else return tagVal + query(lch[p], l, mid, ql, mid) + query(rch[p], mid + 1, r, mid + 1, qr);
}