...
int search(int p, int l, int r, int ql, int qr, int d) {
    if (ql == l && qr == r) {
        ...
        if (l == r) return l;
        int mid = (l + r) >> 1;
        if (tr[p * 2].minv >= d) return search(p * 2, l, mid, ql, mid, d);
        else return search(p * 2 + 1, mid + 1, r, mid + 1, qr, d);
    }
    int mid = (l + r) >> 1;
    if (qr <= mid) return search(p * 2, l, mid, ql, qr, d);
    else if (ql > mid) return search(p * 2 + 1, mid + 1, r, ql, qr, d);
    else {
...
void change(vector<ll>& a, int p, int l, int r, int x, ll val) {
    ...
    int mid = (l + r) >> 1;
    if (x <= mid) change(a, p * 2, l, mid, x, val);
    else change(a, p * 2 + 1, mid + 1, r, x, val);
    update(p);
}