...
Tag operator+(Tag &a, Tag &b) { return {a.mul * b.mul, a.add * b.mul + b.add}; }
struct Node {
  ll sum[2];
  int l, r;
  Tag tag;
...
void setTag(int p, Tag t) {
  tr[p].sum[1] = t.mul * t.mul * tr[p].sum[1] +
                 2 * t.mul * t.add * tr[p].sum[0] +
                 t.add * t.add * (tr[p].r - tr[p].l + 1);
...
void modify(int p, int l, int r, int ql, int qr, Tag t) {
  ...
  int mid = (l + r) >> 1;
  if (qr <= mid)
    modify(p * 2, l, mid, ql, qr, t);
  else if (ql > mid)
    modify(p * 2 + 1, mid + 1, r, ql, qr, t);
  else {
