...
void modify_from(int &now, int pre, int l, int r, int x) {
  ...
  int mid = (l + r) / 2;
  if (x < mid)
    modify_from(tr[now].l, tr[pre].l, l, mid, x);
  else
    modify_from(tr[now].r, tr[pre].r, mid + 1, r, x);
...
LL query(int root1, int root2, int l, int r, int L, int R) {
  ...
  int mid = (l + r) / 2;
  return query(tr[root1].l, tr[root2].l, l, mid, L, R) +
         query(tr[root1].r, tr[root2].r, mid + 1, r, L, R);
}
const int N = 1e9;
