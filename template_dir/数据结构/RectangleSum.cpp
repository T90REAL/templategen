...
class default_srecs{
    ...
    inline static constexpr value_type fx(const value_type &a,const value_type &b){ return a+b;}
    inline static constexpr value_type ix(const value_type &a,const value_type &b){ return a-b;}
};
...
class static_rectangle_sum{
    ...
    struct point{
        ...
        value_type w;
        point(){}
        point(area_type x,area_type y,const value_type &w)
        :x(x),y(y),w(w){}
    };
    ...
    struct answer_type{
        ...
        value_type w;
        answer_type(){}
        answer_type(const int id,area_type x1,area_type y1,area_type x2,area_type y2)
    ...
    inline int add_point(const area_type x,const area_type y,const value_type &w){
        CX.emplace_back(x);
        CY.emplace_back(y);
    ...
    inline int add_query_rectangle(area_type x1,area_type y1,area_type x2,area_type y2){
    ...
    void build(){
        ...
        const auto compx=[&](area_type &x){ x=std::lower_bound(CX.begin(),CX.end(),x)-CX.begin();};
        const auto compy=[&](area_type &y){ y=std::lower_bound(CY.begin(),CY.end(),y)-CY.begin();};
        for(auto &e:P) compx(e.x),compy(e.y);
        for(auto &e:R) compx(e.x2),compy(e.y2);
        std::sort(P.begin(),P.end(),
                [](const point &p,const point &q){ return p.x<q.x||(p.x==q.x&&p.y<q.y);});
        std::sort(R.begin(),R.end(),
                [](const rectangle &r,const rectangle &s){ return r.x2<s.x2||(r.x2==s.x2&&r.y2<s.y2);});
        ...
        for(int i=0;i<sx;++i){
            ...
            while(itrr!=R.end()&&(*itrr).x2==i){
                if((*itrr).sign){
                    ans[(*itrr).id].w=operator_class::fx(ans[(*itrr).id].w,BIT.sum((*itrr).y2+1));
                }else{
                    ans[(*itrr).id].w=operator_class::ix(ans[(*itrr).id].w,BIT.sum((*itrr).y2+1));
                }
                ++itrr;
            }
        }
