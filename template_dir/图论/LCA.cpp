...
struct ST {
  ...
  void load(vector<T>& a) {
    ...
    rep(i, 1, lg(n) + 1) for (int j = 0; j + (1 << i) - 1 < n; j++) st[i][j] =
...
pair<pair<int, int>, int> findDiameter() {
  ...
  int p1, p2;
  p1 = max_element(dist.begin() + 1, dist.end()) - dist.begin();
  que.clear();
  dist.assign(n, 1e9);
  dist[p1] = 0;
  que.push_back(p1);
  ...
  p2 = max_element(dist.begin() + 1, dist.end()) - dist.begin();
  int d = *max_element(dist.begin() + 1, dist.end());
