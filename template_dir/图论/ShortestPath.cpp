namespace Dijstra{
template<class T>vector<pair<bool, T> > max_path(const vector<vector<pair<int, T> > >&edges, const int &s){
...
template<class T>vector<pair<bool, T> > min_path(const vector<vector<pair<int, T> > >&edges, const int &s){
    auto _edges = edges;
    int n = edges.size();
    for (int i = 0; i < n; i ++)
        for (auto &u:_edges[i])
...
template<class T>pair<bool, vector<pair<bool, T> > > max_path(const vector<vector<pair<int, T> > >&edges, const int &s){
    int n = edges.size();
...
template<class T>pair<bool, vector<pair<bool, T> > > min_path(const vector<vector<pair<int, T> > >&edges, const int &s){
    auto _edges = edges;
    int n = edges.size();
    for (int i = 0; i < n; i ++)
        for (auto &u:_edges[i])
...
namespace Johnson{
template<class T>pair<bool, vector<vector<pair<bool, T> > > > min_path(vector<vector<pair<int, T> > > edges){
    int n = edges.size();
    edges.push_back({});
    for (int i = 0; i < n; i ++)
        edges[n].push_back({i, 0});
    auto result = BellmanFord::min_path(edges, n);
    if (!result.first) return {0, {}};
    auto &dis = result.second;
    for (int i = 0; i < n; i ++)    
        for (auto &u:edges[i])
            u.second += (dis[i].second - dis[u.first].second);
    edges.pop_back();
