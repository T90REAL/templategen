// 0_index base
// add edge by making sure the CNF such as x_1 and x_2 and x_3.... (x_i should
// be format as (a or b)) a xor b <=> (a or b) and (-a or -b)
...
#define rep(i, a, b) for (int i = a; i < b; i++)
  ...
  VI valid() {
    ...
    int cnt = 0;
    rep(i, 0, tot) if (comp[order[tot - i - 1]] == -1)
        dfs2(order[tot - i - 1], cnt++);
...
auto twosat = [&]() {
  ...
  for (int i = 0; i < tot; i += 2) {
    ...
    if (!paint(i)) {
      while (!stk.empty())
        ans[stk.back()] = ans[stk.back() ^ 1] = 0, stk.pop_back();
