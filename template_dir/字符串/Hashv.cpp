...
int operator * (int p, Hashv a) {
    return 1ll * p * a.fi % a.se;
}
Hashv operator * (Hashv a, pair<Hashv, Hashv> _base) {
    return mp(a.fi * _base.fi, a.se * _base.se);
}
Hashv operator * (Hashv a, Hashv b) {
    return mp(1ll * a.fi * b.fi % baseMod.fi, 
            1ll * a.se * b.se % baseMod.se);
}
...
auto get = [&](int l, int r) -> Hashv {
    // return mp((hash[r].fi - 1ll * hash[l - 1].fi * pw[r - l + 1].fi % base1.se + base1.se) % base1.se,
    //        (hash[r].se - 1ll * hash[l - 1].se * pw[r - l + 1].se % base2.se + base2.se) % base2.se);
